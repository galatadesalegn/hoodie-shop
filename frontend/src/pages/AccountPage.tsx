            ...
            <AnimatePresence mode="wait">
              {activeTab === 'profile' && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="bg-white dark:bg-white/[0.03] rounded-3xl p-10 shadow-sm border border-noir/5 dark:border-white/5"
                >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 sm:p-10 space-y-10 custom-scrollbar">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  <div className="p-6 rounded-3xl bg-noir/[0.02] dark:bg-white/[0.02] border border-noir/5 dark:border-white/5 h-full">
                    ...
                    {selectedOrder.statusHistory && selectedOrder.statusHistory.length > 0 && (
                      <div className="mt-5 pt-4 border-t border-noir/5 dark:border-white/5 space-y-3">
                        <p className="text-[9px] font-black text-noir/30 dark:text-white/30 uppercase tracking-widest mb-2">Tracking History</p>
                        <div className="space-y-4">
                          {selectedOrder.statusHistory.map((history, i) => (
                            <div key={i} className="flex gap-3 relative">
                              {i !== selectedOrder.statusHistory.length - 1 && (
                                <div className="absolute left-[3px] top-4 bottom-[-16px] w-[2px] bg-noir/5 dark:bg-white/5" />
                              )}
                              <div className="w-2 h-2 rounded-full bg-noir/20 dark:bg-white/20 mt-1.5 flex-shrink-0 z-10" />
                              <div className="flex-1">
                                <p className="text-[10px] font-black text-noir dark:text-white uppercase tracking-tight">{history.status}</p>
                                {history.note && (
                                  <p className="text-[10px] font-bold text-noir/60 dark:text-white/60 mt-0.5 leading-relaxed">{history.note}</p>
                                )}
                                <p className="text-[8px] font-bold text-noir/30 dark:text-white/30 uppercase tracking-widest mt-1">
                                  {new Date(history.changedAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </p>
                              </div>
                            </div>
                          ))}
